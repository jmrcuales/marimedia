...
**Content hierarchy.** Section heading → a short paragraph naming JM and Kristine, their real backgrounds (JM: computer engineering, software engineering, web development, UX, email marketing; Kristine: media sales, affiliate management, and named prior experience per Compass §3), why they started Mari Media, and a plain statement that health is the first focus, not the permanent one → optional link to a fuller About page/section once it exists.
...
5. **Company & Founders copy corrected**, not just polished: the existing body copy never actually named JM or Kristine or stated either founder's real background, despite the section's own heading ("Built by two people, on purpose") and blueprint Section 5.7's explicit content-hierarchy requirement ("a short paragraph naming JM and Kristine, their real backgrounds"). This was a specificity gap against Compass Section 27, not a stylistic one, and was corrected using only the founder facts already recorded in the Compass (Section 3).
